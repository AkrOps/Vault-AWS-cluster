...
# Wait a bit for networking just in case
sleep 5
...
secret_result=$(aws secretsmanager get-secret-value --secret-id ${tls_secret_arn} --region ${region} --output text --query SecretString)
...
# Add CA cert to trusted root CAs
cp /opt/vault/tls/vault-ca.pem /usr/local/share/ca-certificates/vault-ca.crt
...
storage "raft" {
  ...
  node_id = "$instance_id"
  retry_join {
    auto_join = "provider=aws region=${region} tag_key=Vault_cluster tag_value=${cluster_name}"
...
# Install Vault CLI autocomplete for ubuntu user
runuser -l ubuntu -c 'vault -autocomplete-install'
